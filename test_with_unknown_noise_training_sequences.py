...
def test_func(opt, load_model, load_state_params):
    ...
    in_dir = os.path.join(opt.dataset_dir, 'training_sequences')
    ...
    out_seq_dir = os.path.join(model_out_dir, '{}'.format(opt.dataset_name))
    if not os.path.exists(out_seq_dir):
        os.makedirs(out_seq_dir)
    out_clean_dir = os.path.join(out_seq_dir, 'clean')
    if not os.path.exists(out_clean_dir):
        os.makedirs(out_clean_dir)
    out_noisy_dir = os.path.join(out_seq_dir, 'noisy')
    if not os.path.exists(out_noisy_dir):
        os.makedirs(out_noisy_dir)
    pcst_dir = os.path.join(out_seq_dir, 'pcst')
    if not os.path.exists(pcst_dir):
        os.makedirs(pcst_dir)
    pretrained_dir = os.path.join(out_seq_dir, 'pretrained')
    if not os.path.exists(pretrained_dir):
        os.makedirs(pretrained_dir)

    clean_seq_names = sorted(os.listdir(in_clean_dir))
    noisy_seq_names = sorted(os.listdir(in_noisy_dir))
    missing_seq_names = set(clean_seq_names).difference(set(noisy_seq_names))
    assert_message = ("noisy versions of sequences \"{}\" are missing".format(', '.join(missing_seq_names)))
    assert len(missing_seq_names) == 0, f"{assert_message}"
    missing_seq_names = set(noisy_seq_names).difference(set(clean_seq_names))
    assert_message = ("clean versions of sequences \"{}\" are missing".format(', '.join(missing_seq_names)))
    assert len(missing_seq_names) == 0, f"{assert_message}"
    ...
    tl_inc = len(clean_seq_names) // toolbar_width + 1
    head_str = "[test]"
    ...
    for seq_i in range(len(clean_seq_names)):
        name_tmp = clean_seq_names[seq_i]
        in_clean_seq_dir = os.path.join(in_clean_dir, name_tmp)
        clean_fr_names = sorted(os.listdir(in_clean_seq_dir))
        in_noisy_seq_dir = os.path.join(in_noisy_dir, name_tmp)
        noisy_fr_names = sorted(os.listdir(in_noisy_seq_dir))
        missing_fr_names = set(clean_fr_names).difference(set(noisy_fr_names))
        assert_message = ("noisy versions of frames \"{}\" in sequence \"{}\" are missing".format(
            ', '.join(missing_fr_names), clean_seq_names[seq_i]))
        assert len(missing_fr_names) == 0, f"{assert_message}"
        missing_fr_names = set(noisy_fr_names).difference(set(clean_fr_names))
        assert_message = ("clean versions of frames \"{}\"in sequence \"{}\" are missing".format(
            ', '.join(missing_fr_names), clean_seq_names[seq_i]))
        assert len(missing_fr_names) == 0, f"{assert_message}"

        for fr_i in range(len(clean_fr_names)):
            clean_path = os.path.join(in_clean_seq_dir, clean_fr_names[fr_i])
            ...
            noisy_path = os.path.join(in_noisy_seq_dir, clean_fr_names[fr_i])
            ...
            with torch.no_grad():
                ...
                if opt.save_images:
                    reconst_path = os.path.join(reconst_dir, 'im_s{}_f{}.jpg'.format(seq_i, fr_i))
                    ...
                    pretrained_path = os.path.join(pretrained_dir, 'im_s{}_f{}.jpg'.format(seq_i, fr_i))
...
if __name__ == "__main__":
    ...
    - This script evaluates a given denoising architecture with noise with unknown model.
      The script uses for evaluation TRAINING SEQUENCES, as it was done in the paper.
